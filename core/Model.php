...
abstract class Model
{
    ...
    public function validate()
    {
        foreach ($this->rules() as $attribute => $rules) {
            ...
            foreach ($rules as $rule) {
                ...
                if ($ruleName === self::RULE_ACTIVE && !$this->activeUser($value)) {
                    $this->addErrors($attribute, $ruleName);
                }
    ...
    public function errorMessages()
    {
        return [
            ...
            self::RULE_EMAIL_UNIQUE => "E-mail already exists",
            self::RULE_MATCH => "This field must be the same as match",
            self::RULE_ACTIVE => "The user with that e-mail address does not exist"
        ];
    }
    ...
    public function activeUser($email) 
    {
        ...
        $sqlString = "SELECT * FROM user WHERE email = '$email' AND is_active = 1;";
